...
class TestProfileView(APITestCase):
    ...
    def test_profile_update(self, upload_resource: Any) -> None:
        ...
        """
        Profile.objects.create(user=self.user)  # type: ignore[attr-defined]
        data = {
            "bio": "Do I function",
            "image": test_image,
        }
        url = reverse("profile", kwargs={"user": self.user.id})  # type: ignore[attr-defined]
        response = self.client.patch(
            url, data=data, format="json", **self.bearer_token
        )
        import pdb
